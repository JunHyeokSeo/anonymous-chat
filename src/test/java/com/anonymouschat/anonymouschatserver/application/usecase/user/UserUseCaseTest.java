...
class UserUseCaseTest {
	...
	class Register {
		...
		void register_success() throws IOException {
			when(userService.checkNicknameDuplicate("닉네임")).thenReturn(false);
			when(userService.register(any(), any())).thenReturn(1L);

			Long result = userUseCase.register(registerCommand, List.of());
		...
		void register_throws_whenNicknameDuplicated() {
			when(userService.checkNicknameDuplicate("닉네임")).thenReturn(true);

			assertThrows(IllegalStateException.class, () -> userUseCase.register(registerCommand, List.of()));
		}

		@DisplayName("이미지 업로드 개수가 제한을 초과하면 회원가입에 실패한다")
		@Test
		void register_throws_whenImageLimitExceeded() {
			...
			assertThrows(IllegalStateException.class, () -> userUseCase.register(registerCommand, images));
		}
	}
	...
	class Update {
		...
		void update_success() throws IOException {
			List<MultipartFile> images = List.of();
			UpdateUserResult expected = mock(UpdateUserResult.class);

			when(userService.update(updateCommand, images)).thenReturn(expected);

			UpdateUserResult result = userUseCase.update(updateCommand, images);
			assertThat(result).isEqualTo(expected);
		}

		@DisplayName("이미지 업로드 개수가 제한을 초과하면 수정에 실패한다")
		@Test
		void update_throws_whenImageLimitExceeded() {
			List<MultipartFile> images = List.of(
					mock(MultipartFile.class), mock(MultipartFile.class),
					mock(MultipartFile.class), mock(MultipartFile.class)
			);

			assertThrows(IllegalStateException.class, () -> userUseCase.update(updateCommand, images));
		}
		...
		void update_throws_whenServiceThrowsIOException() throws IOException {
			...
			when(userService.update(updateCommand, images)).thenThrow(new IOException("에러"));

			assertThrows(IOException.class, () -> userUseCase.update(updateCommand, images));
		}
	}
